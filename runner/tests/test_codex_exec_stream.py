...
class CodexExecStreamTest(unittest.TestCase):
    ...
    def test_reports_skill_only_when_completed_command_returns_its_contents(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(exit_code, 0)
            events = [json.loads(line) for line in (root / "codex-events.jsonl").read_text().splitlines()]
            loaded = [event for event in events if event["type"] == "skill.read"]
            self.assertEqual(loaded, [{"type": "skill.read", "skill_id": "awesome-api-design",
                                       "source": "command_output"}])
            self.assertEqual(json.loads((root / "read-skills.json").read_text()), ["awesome-api-design"])
            self.assertEqual(json.loads((root / "loaded-skills.json").read_text()), [])

    def test_prompt_document_does_not_count_as_native_skill_load(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            skill = root / "skills" / "awesome-api-design"
            skill.mkdir(parents=True)
