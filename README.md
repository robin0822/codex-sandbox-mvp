...
打开 <http://127.0.0.1:5173>。左侧从服务端加载用户的对话窗口；打开窗口后分页读取完整历史，中间显示多轮提问与回答。运行时将 Codex 原始 reasoning、命令、Skill 与 MCP 事件转换为“理解问题、检索资料、读取内容、处理文件、整理回答”等中文阶段进度，默认不暴露英文推理和完整命令；原始 JSONL 仍保留在技术详情与下载产物中。处理时长与执行进度自动更新并滚动到底。回答使用随页面提供的 Markdown 渲染器，fenced `mermaid` 代码块会通过本地 Mermaid 依赖的 strict 安全模式渲染为架构图卡片，渲染期间显示骨架占位；普通 Markdown 和链接经过 DOMPurify 清理，不依赖外部 CDN。刷新会恢复当前窗口及其历史。新对话在第一次提问时创建，并同时分配独立空白工作区。顶部工作区入口可以浏览和下载当前窗口文件。技能广场和 MCP 广场均支持按用户安装与卸载；知识库仍是预留入口。页面提交时只发送当前问题，最近五轮上下文由 API Manager 从数据库读取。
